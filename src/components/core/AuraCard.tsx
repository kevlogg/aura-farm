...
import { ShieldAlert, Flame, Zap, ThumbsUp, ThumbsDown, Flag } from 'lucide-react';
...
export const AuraCard: React.FC<AuraCardProps> = ({ post, onVote, onReport, isFront }) => {
  ...
  const handleReportSubmit = (postId: string, reason: ReportReason, details?: string) => {
    if (onReport) {
      onReport(postId, reason, details);
    }
  };
    ...
    <>
      ...
      >
        ...
        <button
          ...
          className="absolute top-4 right-4 z-40 p-2 rounded-full bg-black/60 backdrop-blur-md border border-white/20 text-zinc-300 hover:text-rose-400 active:scale-90 transition-all"
      ...
      <ReportModal
        ...
        onReportSubmitted={handleReportSubmit}
      />
    </>
  );
};
